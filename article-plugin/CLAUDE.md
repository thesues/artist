...
- `article-visual-planner` — sonnet, WebSearch, 规划图示、截取参考图；支持三套图示风格预设：`claude`（暖米黄牛皮纸调）、`feishu`（飞书/Lark 低饱和多色）、`feishu-gray`（飞书中性灰阶，纯黑白灰、零色相）。由命令在启动前 AskUserQuestion 询问用户、以 `风格:` 参数注入，全文图示统一该风格。feishu 系另有「文字极简」与「防重叠」强制规则
