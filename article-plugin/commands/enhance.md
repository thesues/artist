...
- **询问图示风格（必做）**：用 `AskUserQuestion` 让用户选择全文图示风格，选项为 `claude`（暖米黄牛皮纸调，默认）、`feishu`（飞书 / Lark 低饱和多色，明亮清爽）、`feishu-gray`（飞书中性灰阶，纯黑白灰、零色相）。用户选定后，整篇文章所有图示统一使用该风格——后续把 `风格: <claude|feishu|feishu-gray>` 作为一行写入 Agent 4（及验证轮如需重绘时）的 prompt。若用户跳过则默认 `claude`。`--resume` 时如已记录过风格则沿用，不再追问。
...
| 4 | 路径：`.article-work/rewrite-round-1/origin.md`；图片目录：`.article-work/img/`（按需 Read）；**`风格: <claude\|feishu\|feishu-gray>`**（用户选定值） | `.article-work/rewrite-round-1/04-visual.md` |
