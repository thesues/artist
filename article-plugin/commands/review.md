...
- **询问图示风格（必做）**：用 `AskUserQuestion` 让用户选择图示风格，选项为 `claude`（暖米黄牛皮纸调，默认）、`feishu`（飞书 / Lark 低饱和多色）、`feishu-gray`（飞书中性灰阶，纯黑白灰）。把 `风格: <claude|feishu|feishu-gray>` 作为一行写入 Agent 4 的 prompt，全文图示统一该风格。用户跳过则默认 `claude`。
