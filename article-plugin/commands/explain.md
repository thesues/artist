...
**询问图示风格（启动 Agent 前必做）：** 用 `AskUserQuestion` 让用户选择讲解稿图示风格，选项为 `claude`（暖米黄牛皮纸调，默认）、`feishu`（飞书 / Lark 低饱和多色）、`feishu-gray`（飞书中性灰阶，纯黑白灰）。用户选定后，全篇图示统一该风格——把 `风格: <claude|feishu|feishu-gray>` 作为一行写入 Agent 4 的 prompt。用户跳过则默认 `claude`。
...
| 4 visual-planner | 路径：`.article-work-explain/source/origin.md`；图片目录：`.article-work-explain/img/`（按需 Read）；**`风格: <claude\|feishu\|feishu-gray>`**（用户选定值） | `.article-work-explain/04-visual.md` |
