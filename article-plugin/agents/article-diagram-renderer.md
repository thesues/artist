...
## 调用方需要在 prompt 中指定的参数

| 参数 | 说明 | 默认（未指定时） |
|------|------|----------------|
| 视觉规划报告路径 | 通常是 `.article-work/rewrite-round-N/04-visual.md` 或 `.article-work-explain/04-visual.md` | `.article-work/rewrite-round-1/04-visual.md` |
| SVG 输出目录 | 落盘 `diagram_N.svg` 的目录 | `.article-work/img/` |

下游命令（`/enhance`、`/explain` 等）必须在 prompt 中显式给出这两个路径，便于复用同一个 Agent 处理不同工作目录。

## 执行步骤

### 步骤 1：读取视觉规划报告

从调用方指定的报告路径（如 `.article-work/rewrite-round-N/04-visual.md` 或 `.article-work-explain/04-visual.md`）读取全文。
...
确保输出目录存在（按调用方指定，例如 `.article-work/img/` 或 `.article-work-explain/img/`），然后对每个 SVG：

1. 用 Write 直接把代码块内容（**不含围栏的三个反引号和 `svg` 标签**，只保留 `<svg ...>...</svg>` 本身）写入 `<输出目录>/diagram_N.svg`
2. 编号 N 从 1 开始，按代码块在报告中的出现顺序
3. 如果 `<输出目录>/diagram_N.svg` 已存在，**直接覆盖**（同一轮重复渲染需要可重入）
...
```bash
xmllint --noout "<输出目录>/diagram_N.svg"
```
...
|------|------|---------|-------------|--------------|------|
| 1 | <输出目录>/diagram_1.svg | [名称] | [位置] | `![名称](../img/diagram_1.svg)` | ✅ / ⚠️ xmllint 失败 / — 未校验 |
...
- **所有 SVG 输出到调用方指定的目录**（`/enhance` 是 `.article-work/img/`，`/explain` 是 `.article-work-explain/img/`），与 PDF 提取图 `fig_N.*` 并列
- **命名约定固定为 `diagram_N.svg`**（N 从 1 起）；不得使用语义化命名，rewriter 和 enhance.md / explain.md 的清单依赖该命名
